...
class DQNAgent(object):
  def __init__(self,state_size,action_size):
    ...
    self.memory=PrioritizedReplayBuffer(state_size[1],action_size,size=7000)
    self.gamma=0.99
    self.epsilon=1
  ...
  def _train_step(self,states,actions,rewards,next_states,done,weights):
    with tf.GradientTape() as tape:
        ...
        best_next_actions=tf.argmax(q_next_main,axis=1,output_type=tf.int32)
        q_next_target=self.target_model(next_states)
        batch_indices=tf.range(tf.shape(next_states)[0],dtype=tf.int32)
        next_action_indices=tf.stack([batch_indices,best_next_actions],axis=1)
        next_q_values=tf.gather_nd(q_next_target,next_action_indices)
