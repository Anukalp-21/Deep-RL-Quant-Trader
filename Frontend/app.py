...
# Market feature extraction (unchanged)
...
with st.form("prediction_form"):
    ...
    cash = st.number_input("Cash (₹)", 
                           min_value=0.0, 
                           value=float(st.session_state.portfolio['cash']),
                           step=1000.0)
    
    date = st.date_input("Select Date", datetime.today())
    
    # Fixed submit button usage
    submitted = st.form_submit_button("Get Trading Action")
...
# Test Simulation Section (unchanged)
